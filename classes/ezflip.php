...
class ezFlip
{
    ...
    public static function get_symlink_flip_dir()
    {
        $ini = eZINI::instance( 'site.ini' );
		$var = $ini->variable( 'FileSettings','VarDir' );
        $relative_var = basename( $var );
        return $relative_var;
    }
    
    public static function flipBaseFolderPath()
    {
        $ini = eZINI::instance( 'site.ini' );
		$var = $ini->variable( 'FileSettings','VarDir' );
        $relative_var = basename( $var );

        $flip_dir_base = $var . '/storage/original/application_flip';
        eZDir::mkdir( $flip_dir_base );
    
        $megazine_dir_symlink =  'extension/ezflip/design/standard/flash/megazine';
        
        if ( !file_exists( $megazine_dir_symlink . '/' . $relative_var ) )
        {
            $sourceDirPos = explode( '/', $megazine_dir_symlink );
            $relative_slashes = '';
            for ($i = 1; $i <= count( $sourceDirPos ) ; $i++)
            {
                $relative_slashes .= '../';
            }
            symlink( $relative_slashes . $flip_dir_base,  $megazine_dir_symlink . '/' . $relative_var );
        }
        return $flip_dir_base;
    }
    ...
    public static function flipFolderPath ( $object_id = 0 ) 
	{
        ...
        $flip_dir_base = self::flipBaseFolderPath();
		$flip_dir = $flip_dir_base . '/' . $object_id;
    ...
    {
        $filename = self::flipFolderPath( $object_id );
        $ini = eZINI::instance( 'ezflip.ini' );		
		$books = $ini->variable( 'FlipBookSettings', 'FlipBook');		
		foreach ( $books as $book )
        {
            $file = eZClusterFileHandler::instance( $filename . "/magazine_" . $book . ".xml" );
            if ( $file->exists() )
            {
                return true;
            }
